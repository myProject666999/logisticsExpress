...
const SendManagement = () => {
  ...
  const [filters, setFilters] = useState({ tracking_number: '', sender: '', status: '' })
  ...
  const handleShip = async (record) => {
    try {
      await request.post(`/packages/${record.id}/ship`)
  ...
  const columns = [
    ...
    { title: '寄件电话', dataIndex: 'sender_phone', key: 'sender_phone' },
  ...
  const handleReset = () => {
    setFilters({ tracking_number: '', sender: '', status: '' })
    loadData(1, pagination.pageSize)
  }
